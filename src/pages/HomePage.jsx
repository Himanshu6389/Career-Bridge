...
function HomePage() {
  ...
  const [step, setStep] = useState(1); // 1 = email, 2 = payment, 3 = verify
  ...
  useEffect(() => {
    const saved = localStorage.getItem('paymentVerified');
    if (saved === 'true') {
      setAccessGranted(true);
    }
  }, []);

  // Payment Details - UPDATE THIS WITH YOUR INFO
  const payment = {
    upiId: '9934597795-2@ybl',
    amount: 1000,
    accountHolder: 'Samir suman',
    accountNumber: '1234567890123456',
    ifscCode: 'ICIC0000001',
    verificationCode: 'SS2025PAID'
  };
  ...
  const copyToClipboard = (text, type) => {
    ...
    setCopyNotif(`${type} copied!`);
    setTimeout(() => setCopyNotif(''), 2000);
  };

  const handleVerifyCode = (e) => {
    e.preventDefault();
    if (code.trim() === payment.verificationCode) {
  ...
  // AFTER PAYMENT - Show the original homepage here
  if (accessGranted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        ...
        <main className="max-w-6xl mx-auto px-4 py-12">
          <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-3xl font-bold text-green-600">Payment Verified! ✅</h2>
            </div>
            <p className="text-gray-600 text-lg mb-4">
              Thank you for the payment of ₹{payment.amount}! Your project is now unlocked.
            </p>
          ...
          {/* Your Original Homepage Code Goes Here */}
          {/* UNCOMMENT YOUR ORIGINAL CODE BELOW */}
          
          {/*
          
          ========== PASTE YOUR ORIGINAL HOMEPAGE CODE HERE ==========
          
          Example:
          <section>
            <h2 className="text-4xl font-bold mb-6">Project Features</h2>
          ...
          <section className="mt-12">
            ...
            <ul className="space-y-3">
              <li className="flex items-center gap-3">
                <span className="text-green-500">✓</span> Your project files
              </li>
              <li className="flex items-center gap-3">
                <span className="text-green-500">✓</span> Full documentation
              </li>
              <li className="flex items-center gap-3">
                <span className="text-green-500">✓</span> Support
              </li>
            </ul>
          </section>
          ...
          */}
          ...
          <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg p-12 text-center">
            <h2 className="text-4xl font-bold mb-4">Your Original Homepage Will Appear Here</h2>
            <p className="text-xl opacity-90 mb-6">
              After receiving payment, uncomment your original homepage code above this section and redeploy!
            </p>
            <p className="text-sm opacity-75">
              Currently showing: Payment verification screen only
            </p>
          </div>
        </main>
        ...
        <footer className="bg-gray-800 text-white mt-12 py-6 text-center">
          <p>© 2025 Your Project | Payment ✓</p>
        </footer>
      </div>
    );
  }

  // BEFORE PAYMENT - Show payment screen only
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Payment Required</h1>
          <p className="text-gray-300">Complete payment to access project</p>
        </div>

        {/* Step 1: Email */}
        {/* {step === 1 && (
          <div className="p-8 bg-slate-800 rounded-lg border border-slate-700 shadow-xl">
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-4">
              <input
                ...
                className="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded text-white placeholder-gray-400 focus:border-blue-500 focus:outline-none"
                required
              />
              <button
                type="submit"
                className="w-full px-6 py-3 bg-blue-500 text-white font-bold rounded hover:bg-blue-600 transition"
              >
                Continue to Payment
              </button>
            </form>
        ...
        {step === 2 && (
          <div className="p-8 bg-slate-800 rounded-lg border border-slate-700 shadow-xl">
            ...
            <div className="mb-6 p-4 bg-blue-500/20 border border-blue-500 rounded-lg">
              <p className="text-gray-300 text-sm">Amount to Pay</p>
              <p className="text-4xl font-bold text-blue-400">₹ {payment.amount}</p>
            </div>

            {/* UPI */}
            <div className="mb-6 p-4 bg-slate-700 rounded-lg">
              <h3 className="text-lg font-bold text-white mb-3">📱 UPI Payment</h3>
              <div className="flex items-center justify-between bg-slate-600 p-3 rounded">
                <code className="text-blue-400 font-mono">{payment.upiId}</code>
                <button
              ...
              <p className="text-xs text-gray-400 mt-2">Use Google Pay, PhonePe, or Paytm</p>
            </div>

            {/* Bank */}
            {/* <div className="mb-6 p-4 bg-slate-700 rounded-lg">
              <h3 className="text-lg font-bold text-white mb-3">🏦 Bank Transfer</h3>
              <div className="space-y-2">
                <div className="flex items-center justify-between bg-slate-600 p-3 rounded">
                  <span className="text-sm text-gray-300">Account</span>
                  <button
                    onClick={() => copyToClipboard(payment.accountNumber, 'Account')}
                    className="text-blue-400 text-sm font-mono hover:text-blue-300"
                  >
                    {payment.accountNumber} <Copy size={14} className="inline" />
                  </button>
                </div>
                <div className="flex items-center justify-between bg-slate-600 p-3 rounded">
                  <span className="text-sm text-gray-300">IFSC</span>
                  <button
                    onClick={() => copyToClipboard(payment.ifscCode, 'IFSC')}
                    className="text-blue-400 text-sm font-mono hover:text-blue-300"
                  >
                    {payment.ifscCode} <Copy size={14} className="inline" />
                  </button>
                </div>
              </div>
            </div> */}
            ...
            {/* Copy notification */}
            {copyNotif && (
              <div className="mb-4 p-3 bg-green-500/20 border border-green-500 rounded text-green-400 text-sm flex items-center gap-2">
                <CheckCircle size={16} />
                {copyNotif}
              </div>
            )}
            ...
            <button
              onClick={() => setStep(3)}
              className="w-full px-6 py-3 bg-green-500 text-white font-bold rounded hover:bg-green-600 transition"
            >
              Paid? Verify Now →
            </button>
          </div>
        ...
        {step === 3 && (
          <div className="p-8 bg-slate-800 rounded-lg border border-slate-700 shadow-xl">
            ...
            <p className="text-gray-300 mb-4">Enter verification code sent to your email</p>
            <form onSubmit={handleVerifyCode} className="space-y-4">
              <input
                ...
                className="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded text-white placeholder-gray-400 focus:border-blue-500 focus:outline-none font-mono text-center text-lg"
                required
              />
              <button
                type="submit"
              ...
              <button
                type="button"
                onClick={() => setStep(2)}
                className="w-full px-6 py-3 bg-slate-700 text-white font-bold rounded hover:bg-slate-600 transition"
              >
                Back
              </button>
            </form>
            <p className="text-xs text-gray-400 mt-4 text-center">
              💡 Test code: GK2025PAID
            </p>
          </div>
        )}

        {/* Demo Info */}
        {/* <div className="mt-6 p-4 bg-yellow-500/10 border border-yellow-500/50 rounded-lg text-center text-yellow-400 text-sm">
          For testing: Use code <strong>GK2025PAID</strong>
        </div> */}
      </div>
    </div>
  );
